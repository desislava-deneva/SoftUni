class ArtGallery1 {
    ...
    possibleArticles: { picture: number; photo: number; item: number; };
    listOfArticles: any[];
    guests: any[];
    ...
    buyArticle(articleModel: string, articleName: string, guestName: string): string {

        let isExist = false;
        const test: MyOject = null;
        for (const key of Object.keys(this.possibleArticles)) {
            if (key === articleModel) {
                isExist = true;
                break;
...
const artGallery1 = new ArtGallery1('Curtis Mayfield');
...
artGallery1.buyArticle('picture', 'Mona Liza', 'John');
artGallery1.buyArticle('item', 'Ancient vase', 'Peter');
console.log(artGallery1.showGalleryInfo('article'));
console.log(artGallery1.showGalleryInfo('guest'));
